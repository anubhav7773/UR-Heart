...
async def get_meetup_spots(
    ...
    radius_meters: Optional[int] = Query(None, ge=500, le=50000, description="Optional search radius in meters (max 50km hard ceiling)"),
    ...
    - 3-Stage Tiered Radius Search (15km -> 35km -> 50km hard ceiling).
    - In-Memory TTL Caching (6 hours) & resilient Overpass mirror fallback.
    - Aggressive commercial quality filtering & junk rejection.
    - Produces high-reliability Google Maps direct business coordinates.
    """
    is_midpoint = False
    user_dist_km: Optional[float] = None
    target_radius = radius_meters or 50000
